...
void arangodb::aql::distributeInClusterRule(Optimizer* opt,
                                            ...
                                            OptimizerRule const* rule) {
  bool wasModified = false;
  std::vector<std::pair<ExecutionNode*,ExecutionNode*>> subQueryEndNodePairs;
  ...
  if (arangodb::ServerState::instance()->isCoordinator()) {
    ...
    for (ExecutionNode* root : nodes) {
      ...
      if (collInfo->isSmart() && collInfo->type() == TRI_COL_TYPE_EDGE) {
        distributeInClusterRuleSmartEdgeCollection(
            plan.get(), node, root, originalParent, subQueryEndNodePairs, wasModified);
        continue;
      }
#endif
      ...
      if (originalParent != nullptr) {
        ...
        auto planRoot = plan->root();  // why dont we just use root?
        plan->unlinkNode(node, true);
        if (!isSubQuery){
          plan->root(planRoot, true);  // fix root node - replace root with root?
        } else {
          // update root in subquery
          std::pair<ExecutionNode*,ExecutionNode*> nodepair = std::make_pair(node,originalParent);
          subQueryEndNodePairs.emplace_back(std::move(nodepair));
          root=originalParent;
        }
      } else {
        ...
        if (!isSubQuery){
          ...
          std::pair<ExecutionNode*,ExecutionNode*> nodepair = std::make_pair(node,deps[0]);
          subQueryEndNodePairs.emplace_back(std::move(nodepair));
          root=deps[0];
        }
      }
      ...
      if (originalParent != nullptr) {
        ...
        if (!isSubQuery){
          ...
          std::pair<ExecutionNode*,ExecutionNode*> nodepair = std::make_pair(root,gatherNode);
          subQueryEndNodePairs.emplace_back(std::move(nodepair));
        }
      }
      wasModified = true;
    ...
    // update entry points of subquieries if the root of subqueries has been modified
    SmallVector<ExecutionNode*>::allocator_type::arena_type b;
    SmallVector<ExecutionNode*> subqueryNodes{b};
    plan->findNodesOfType(subqueryNodes, ExecutionNode::SUBQUERY, true);
    for(auto const& pair : subQueryEndNodePairs){
      for(ExecutionNode* enode : subqueryNodes){
        auto snode = static_cast<SubqueryNode*>(enode);
        if (snode->getSubquery() == pair.first) {
          snode->setSubquery(pair.second, true);
        }
      }
    }
